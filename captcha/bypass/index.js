...
export default class {
    ...
    async waitForResponse() {
        const { body } = await this.session.get('https://ipv4.icanhazip.com/');
        const ip = body.trim();
            ...
            try {
                ...
                const getTopLevel = (time) => {
                    return {
                        ...
                        nv: {
                            ...
                            appCodeName: 'Mozilla',
                            appName: 'Netscape',
                            appVersion: '"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.87 Safari/537.36',
                            platform: 'Linux i686',
                            product: 'Gecko',
                ...
                const { json: captcha } = await this.session.post(`https://hcaptcha.com/getcaptcha?s=${this.sitekey}`, new URLSearchParams({
                    ...
                    motionData: JSON.stringify({
                        ...
                        href: 'file:///ios_asset/hcaptcha-form.html',
                        prev: {
                            escaped: false,
                            passed: false,
